...
def get_optimizer(hparams, models):
    ...
    parameters = []
    for model in models:
        parameters += nn.ParameterList(model.parameters())
    if hparams.optimizer == 'sgd':
        optimizer = SGD(parameters, lr=hparams.lr_NeRF, 
                        momentum=hparams.momentum, weight_decay=hparams.weight_decay)
    elif hparams.optimizer == 'adam':
        optimizer = Adam(parameters, lr=hparams.lr_NeRF, eps=eps, 
                         weight_decay=hparams.weight_decay)
    elif hparams.optimizer == 'radam':
        optimizer = RAdam(parameters, lr=hparams.lr_NeRF, eps=eps, 
                          weight_decay=hparams.weight_decay)
    elif hparams.optimizer == 'ranger':
        optimizer = Ranger(parameters, lr=hparams.lr_NeRF, eps=eps, 
                          weight_decay=hparams.weight_decay)
    else:
...
def get_optimizer1(hparams, Ir_tensor, models, i):
    eps = 1e-8
    parameters = []
    for model in models:
        parameters += list(model.parameters())
    optimizer = Adam(model.parameters(), lr=Ir_tensor[i], betas=(hparams.beta, 0.999))
    # if hparams.optimizer == 'sgd':
    #     optimizer = SGD(parameters, lr=hparams.lr_NeRF, 
    #                     momentum=hparams.momentum, weight_decay=hparams.weight_decay)
    # elif hparams.optimizer == 'adam':
    ...
    return optimizer
def get_optimizer2(hparams, models):
    eps = 1e-8
    parameters = []
    for model in models:
        parameters += list(model.parameters())
    optimizer = Adam(model.parameters(), lr=hparams.lr_NeRV, betas=(hparams.beta, 0.999))
    # if hparams.optimizer == 'sgd':
    #     optimizer = SGD(parameters, lr=hparams.lr_NeRF, 
    #                     momentum=hparams.momentum, weight_decay=hparams.weight_decay)
    # elif hparams.optimizer == 'adam':
