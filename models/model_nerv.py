...
class CustomDataSet(Dataset):
    def __init__(self, main_dir, transform, vid_list=[None], frame_gap=1,  visualize=False, time_change = 0):
        ...
        for img_id in all_imgs:
            self.frame_path.append(img_id)   #和all_imgs是一样的
            frame_idx.append(num_frame)
            num_frame += 1          

        # import pdb; pdb.set_trace; from IPython import embed; embed()
        accum_img_num.append(num_frame)
        self.frame_idx = [float(x) / len(frame_idx) + self.time_change for x in frame_idx]
    ...
    def __getitem__(self, idx):
        ...
        frame_idx = torch.tensor(self.frame_idx[valid_idx] + self.time_change)
...
class Generator(nn.Module):
    ...
    def forward(self, input):
        # pdb.set_trace()
        # print("ok")
        output = self.stem(input)   # [1, 80]——>[1, 3744]  26*h*w 9 16
        output = output.view(output.size(0), self.fc_dim, self.fc_h, self.fc_w)

        out_list = []
        for layer, head_layer in zip(self.layers, self.head_layers):
            output = layer(output)   #[1, 26, 9*5, 16*5]   [1, 96, 90, 160]
            if head_layer is not None:
                img_out = head_layer(output)  #[1, 96, 720, 1280]   [1, 3, 720, 1280]
